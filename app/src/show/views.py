...
def song_detail_list(request, show_id=None, run_id=None):
    ...
    for song in show.song_set.all():
        ...
        ensemble_actor_list = [
            x for x in ensemble_actor_list if x not in featured_actor_list
        ]

        featured_characters = song.featured_characters.all()
        ensemble_characters = song.ensemble_characters.all()
        ensemble_characters = [
            x for x in ensemble_characters if x not in featured_characters
        ]

        featured_groups = song.featured_groups.all()
        ensemble_groups = song.ensemble_groups.all()
        ensemble_groups = [
            x for x in ensemble_groups if x not in featured_groups
        ]
        songs.append(
            {
                **model_to_dict(song),
    ...
    return render(
        request,
        "show/songsummary.html",
        {"show": show, "run": run, "songs": songs},
    )
...
def minimal_mic_character_list_by_actor(request, show_id=None, run_id=None):
    ...
    character_assignments = (
        CharacterInShow.objects.filter(run=run)
        .select_related("primary_actor")
        .order_by("primary_actor__name")
    )
    ...
    for ca in character_assignments:
        if last_character != ca.primary_actor:
            if add:
                mic = MicrophoneAssignment.objects.filter(
                    show=show, run=run, character__in=mic_list
                )
                logger.warn(mic)
                if mic:
                    mic_assigned.append(
    ...
    if add:
        if mic:
            mic_assigned.append(
                {"actor": last_character, "micable_characters": mic_list}
            )
        else:
            nomic_assigned.append(
                {"actor": last_character, "micable_characters": mic_list}
...
def micindex(request, show_id=None, run_id=None):
    ...
    return render(request, "show/micindex.html", {"show": show, "run": run})
