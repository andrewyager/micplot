...
class MicModel(models.Model):
    manufacturer = models.ForeignKey(
        "microphone.Manufacturer", on_delete=models.CASCADE
    )
    name = models.CharField(max_length=255)
...
class Microphone(models.Model):
    ...
    model = models.ForeignKey(
        "microphone.MicModel", on_delete=models.CASCADE, blank=True, null=True
    )
    type = models.ForeignKey("microphone.MicType", on_delete=models.CASCADE)
    rx_band = models.ForeignKey(
        RxBand, blank=True, null=True, on_delete=models.CASCADE
    )

    def __str__(self):
        if self.model is not None:
            if self.rx_band is not None:
                return "Mic {} - {} (Band {})".format(
                    self.rx_number, self.model, self.rx_band
                )
            else:
...
class MicrophoneAssignment(models.Model):
    ...
    microphone = models.ForeignKey(
        "microphone.Microphone", on_delete=models.CASCADE
    )
